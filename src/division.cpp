...
namespace division_direction{
    _vec<double> angles(Organ* p_g, int ci){
        ...
        if(p_g->p_c[ci]->IsEpidermal==0){
            if(in_division_direction=="random"){
                ...
                division_direction::asymmetrical_bias_left_right(p_g,ci,asymmetrical_bias_left_gaussian_beta,asymmetrical_bias_right_gaussian_beta,asymmetrical_bias_left_gaussian_phi,asymmetrical_bias_right_gaussian_phi,asymmetrical_bias_left_right_boundary);
            }
    ...
    _vec<double> biregion_angles_position(Organ* p_g, int ci, double bias_beta_apical, double bias_beta_basal, double bias_phi_apical, double bias_phi_basal, double bias_y_boundary){
        double y_max_tmp = organ_geo::organ_maximum_y(p_g);
        double y_min_tmp = organ_geo::organ_minimum_y(p_g);
    ...
    _vec<double> asymmetrical_bias_left_right(Organ* p_g,int ci,double asymmetrical_bias_left_gaussian_beta, double asymmetrical_bias_right_gaussian_beta, double asymmetrical_bias_left_gaussian_phi, double asymmetrical_bias_right_gaussian_phi, double asymmetrical_bias_left_right_boundary){
        organ_geo::organ_center(p_g);
        if(p_g->p_c[ci]->center.x<p_g->center.x){
            //this cell belong to the left region
            std::cout<<"Cell position x "<<p_g->p_c[ci]->center.x<<"; organ center "<<p_g->center.x<<"; this cell belong to left region, its angle bias is ("<<asymmetrical_bias_left_gaussian_beta<<","<<asymmetrical_bias_left_gaussian_phi<<")"<<std::endl;
            Gaussian_bias(p_g,ci,asymmetrical_bias_left_gaussian_beta,asymmetrical_bias_left_gaussian_phi);
        }
        else{
            //this cell belong to the right region
            std::cout<<"Cell position x "<<p_g->p_c[ci]->center.x<<"; organ center "<<p_g->center.x<<"; this cell belong to right region, its angle bias is ("<<asymmetrical_bias_right_gaussian_beta<<","<<asymmetrical_bias_right_gaussian_phi<<")"<<std::endl;
            Gaussian_bias(p_g,ci,asymmetrical_bias_right_gaussian_beta,asymmetrical_bias_right_gaussian_phi);
        }
        return p_g->p_c[ci]->axis;
    }
}
...
namespace division{
    ...
    void Global(Organ *p_g){
        ...
        for(int ci=0;ci<p_g->p_c.size();ci++){
            ...
            if(p_g->p_c[ci]->cellTime>standard_cell_period_length*p_g->p_c[ci]->frequency_modifier){
                ...
                if(division_judge_tmp<F_apparent){
                    ...
                    cell_division_count++;
                    if(p_g->p_c[ci]->IsEpidermal==1){
                        std::cout<<"Cell "<<ci<<" divided"<<"; division frequency: "<<1.0/p_g->p_c[ci]->frequency_modifier<<"; division angle: "<<p_g->p_c[ci]->axisTheta<<endl;
        ...
        std::cout<<"Now, we have "<<(int)p_g->p_c.size()-cell_division_count<<" + "<<cell_division_count<<endl;
